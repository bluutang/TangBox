...
# ---------------------------------------------------------------------------
# RESUMED 2026-08-29. The original run-queue15.sh died at 12:12 with a bogus
# syntax error on line 93: the file was EDITED at 01:05 while it was running,
# and bash reads a script incrementally by byte offset. run_show had been busy
# for 11.5 hours; when it returned, bash resumed at an offset that no longer
# matched the edited file and landed mid-line.
#
#   NEVER EDIT A SHELL SCRIPT THAT IS CURRENTLY RUNNING. Copy it and edit the
#   copy - which is exactly what this file is.
#
# Everything up to and including the geo-blocked Sailor Moon seasons is done.
# Sailor Moon finished at 125, not the expected 127: ArqcS5Sm-FU and 7QszDS4e9mM
# are "Video unavailable" - removed from YouTube, NOT geo-blocked, so no VPN or
# retry brings them back. 125 is the real ceiling.
# ---------------------------------------------------------------------------
