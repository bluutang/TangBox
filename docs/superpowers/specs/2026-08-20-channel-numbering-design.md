...
**Date:** 2026-08-20
**Status:** Structure decided. Numbering recommended but not chosen. Guide scrolling not built.
...
- **How scrolling works.** Cursor-driven (grid scrolls as the cursor passes the
  edge) or paged (a screenful at a time)? Cursor-driven is smoother; paged is
  easier for a child to understand because the tiles stop moving.
